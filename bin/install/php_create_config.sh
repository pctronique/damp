...
sed -i "s/#- .\/.docker\/php\/php.ini:\/usr\/local\/etc\/php\/conf.d\/php.ini:ro/- .\/.docker\/php\/php.ini:\/usr\/local\/etc\/php\/conf.d\/php.ini:ro/" "$REAL_PATH/../../docker-compose.yml" 

docker exec $NAME_SERVER_CONTAINER bash -c "cp /usr/local/apache2/conf/httpd.conf /usr/local/apache2/tmp/httpd"
cp "$REAL_PATH/../../projecttmp/tmp/httpd/httpd.conf" "$REAL_PATH/../../.docker/apache/httpd.conf-example"
cp "$REAL_PATH/../../projecttmp/tmp/httpd/httpd.conf" "$REAL_PATH/../../.docker/apache/httpd.conf"
patch "$REAL_PATH/../../.docker/apache/httpd.conf" < "$REAL_PATH/../../.docker/apache/httpd_conf.patch"

sed -i "s/#- .\/.docker\/apache\/httpd.conf:\/usr\/local\/apache2\/conf\/httpd.conf/- .\/.docker\/apache\/httpd.conf:\/usr\/local\/apache2\/conf\/httpd.conf/" "$REAL_PATH/../../docker-compose.yml"
